...
int main (int argc, char *argv[]) {
  ...
  ApplicationContainer serverApps_1 = echoServer_1.Install (csmaNodes.Get (nCsma));
  ...
  UdpEchoClientHelper echoClient_1 (csmaInterfaces.GetAddress (nCsma), 10); //get ip and port number of server
  echoClient_1.SetAttribute ("MaxPackets", UintegerValue (1)); 
  echoClient_1.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
  echoClient_1.SetAttribute ("PacketSize", UintegerValue (512));
  ...
  ApplicationContainer clientApps_1 = echoClient_1.Install (p2pNodes.Get (0)); //client is n0
  ...
  pointToPoint.EnablePcap ("assignment1a", p2pDevices.Get (0), true);
  pointToPoint.EnablePcap ("assignment1a", p2pDevices.Get (1), true);
  csma.EnablePcap ("assignment1a", csmaDevices.Get (0), true);

  if (nCsma >= 3) {
    csma.EnablePcap ("assignment1a", csmaDevices.Get (3), true);
  }
  ...
  AnimationInterface anim("assignment1a.xml");
  ...
  pointToPoint.EnableAsciiAll(ascii.CreateFileStream("assignment1a.tr"));
