...
def quad_sieve(n, b) :
	...
	lines.append("Values of a close to n^(0.5): " + ','.join([str(f) for f in a]))
	...
	lines.append("x and y chosen such that x^2 is congruent to y^2(mod n): " + str(x) + "," + str(y))

	f = int(gcd(x - y, n))
	g = int(gcd(x + y, n))
	
	if(f!=1 and g!=1):
		lines.append("Final factors f and g such that f = gcd(x+y,n), g = gcd(x-y,n), n = f*g and 1 < (f,g) < n: " + str(f) + "," + str(g) + "\n")
	else:
		lines.append("Unable to determine f and g such that f = gcd(x+y,n), g = gcd(x-y,n), n = f*g and 1 < (f,g) < n.\n")
