...
-- SELECT Patient.Name FROM Patient INNER JOIN PhysicianE ON Patient.Pcp = PhysicianE.EmployeeID WHERE PhysicianE.EmployeeID NOT IN (SELECT Head FROM Department);
...
SELECT Name FROM PhysicianE ph, Undergoes un, Trained_In ti WHERE ph.EmployeeID = un.Physician AND ph.EmployeeID = ti.Physician AND un.DateUndergoes > ti.CertificationExpires;