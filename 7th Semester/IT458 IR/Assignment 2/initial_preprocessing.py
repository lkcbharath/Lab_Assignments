...
def loader():
    filenames = glob("./original_articles/*.txt")
    total_tokens = {}
    file_mapping = {}
...
def construct_inverted_index(token_dict, document_index_mapping):
    ...
    for filename, words in token_dict.items():
        ...
        for word in words:
            inverted_index[word][file_index] += 1
    
    return inverted_index

def save_inverted_index_to_file(inverted_index,filename):
    with open(filename, "w") as f:
        for word, indice_dict in inverted_index.items():
            indice_string = ""
            for key, value in indice_dict.items():
                indice_string += "(" + str(key) + "," + str(value) + ") "
            f.write(word + " : " + indice_string + "\n")
